...
if (function_exists('add_theme_support'))
{
	...
	add_image_size('medium', 550, '', true);
	add_image_size('small', 250, '', true);
	add_image_size('xs', 120, '', true);
	...
	add_image_size('banner', 1500, 650, true);
...
function html5blank_header_scripts()
{
	if ($GLOBALS['pagenow'] != 'wp-login.php' && !is_admin()) {
		...
		wp_register_script('jquery', get_template_directory_uri() . '/scripts/vendor/plugins.min.js', false, '1.0.0'); // Custom scripts
...
function html5blank_styles()
{
	...
	wp_register_style('roboto', 'http://fonts.googleapis.com/css?family=Roboto:400,100,100italic,300,300italic,400italic,500,500italic,700,700italic,900,900italic', array(), '1.0', 'all');
	wp_enqueue_style('roboto');

	wp_register_style('app', get_template_directory_uri() . '/style.css', array(), '1.0.4', 'all');
...
function nl2p($string,$ignoreblank=true,$p='p'){
	$return = '';
	foreach (explode("\n", $string) as $line){
		if (trim($line)){
			$return .= '<'.$p.'>' . $line . '</'.$p.'>';
...
function excerpt_more(){
	return '<a href="' . get_permalink_within_archive() . '">>>>></a>';
}
function short_excerpt(){
	html5wp_excerpt('excerpt_length','excerpt_more');
}

function image_url_from_metabox($field,$postID,$size='large'){
	//single image only (not for multiple images from one CMS input)
	$image = rwmb_meta( $field, array('type'=>'image_advanced', 'size'=>$size), $postID );
...
function wedding_taxonomy_init() {
...
add_action( 'init', 'wedding_taxonomy_init' );

function wedding_post_types()
{

	//FAQ
	register_post_type('faq', // Register Custom Post Type - ARTIST
		array(
		'labels' => array(
			'name' => __('FAQs', 'html5blank'), // Rename these to suit
			'singular_name' => __('FAQ', 'html5blank'),
			'add_new' => __('Add New', 'html5blank'),
			'add_new_item' => __('Add New FAQ', 'html5blank'),
			'edit' => __('Edit', 'html5blank'),
			'edit_item' => __('Edit FAQ', 'html5blank'),
			'new_item' => __('New FAQ', 'html5blank'),
			'view' => __('View FAQ', 'html5blank'),
			'view_item' => __('View FAQ', 'html5blank'),
			'search_items' => __('Search FAQ', 'html5blank'),
...
add_action('init', 'wedding_post_types');
...
include 'admin/theme-config.php';
